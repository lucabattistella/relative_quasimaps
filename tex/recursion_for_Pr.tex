...
Since it has a fundamental class, we can define \emph{relative quasimap invariants} for the pair $(\PP^N,H)$:
\begin{equation*} \langle \ev_1^* \gamma_1 \psi_1^{k_1} , \ldots, \ev_n^* \gamma_n \psi_n^{k_n} \rangle_{0,\alpha,d}^{\PP^N|H} := \int_{[\Q{0}{\alpha}{\PP^N|H}{\beta}]} \prod_{i=1}^n \ev_i^* \gamma_i \psi_i^{k_i} \end{equation*}

We will now prove a number of properties about the fundamental class. These immediately imply corresponding properties about the relative invariants.
...
\]
cuts out $\Q{0}{\alpha+e_k}{\PP^N|H}{d}$ inside $\om{Q} = \Q{0}{\alpha}{\PP^N|H}{d}$, along with a number of degenerate contributions (called the \ilemph{comb loci}) parametrising quasimaps for which $x_k$ belongs to an internal component $Z \subseteq C$ (a component on which $u_0$ vanishes), such that
