...
\end{align*}
Notice that $P_0^X(q)$ depends on the divisor class of $Y$; the superscript is supposed to indicate that the definition only involves quasimap invariants of $X$.
\end{thm}
...
Here we briefly explain how to compare Theorem \ref{Theorem Quantum Lefschetz} to a genus zero wall-crossing formula obtained by Ciocan-Fontanine and Kim. In \cite[Section 5]{CF-K-wallcrossing} they introduce (in the more general context of $\epsilon$-stable quasimaps) the following generating functions for quasimap invariants of $Y$.
\begin{enumerate}
...
where $\QGe{0}{m}{Y}{\beta}$ is the moduli space of $\epsilon$-stable parametrised quasimaps, $F_0$ is a certain fixed locus of the natural $\Gm$-action on this space, and $\ev_\bullet$ is the evaluation at the point $\infty$ on the parametrised component. $\operatorname{Res}_{F_0}$ is the residue of the virtual class, i.e. the induced virtual class on the fixed locus divided by the Euler class of the virtual normal bundle (see \cite{GraberPandharipande} for details on virtual localisation).
...
\begin{equation*}
 P^\epsilon({\bf t}, z)=\sum_{h=1}^k \rho^h \sum_{m\geq 0,\beta\geq 0} \frac{q^\beta }{m!} \left( \ev_1^*(\rho_h \cdot p_\infty) \cap \virt{\QGe{0}{1+m}{Y}{\beta}} \right) \end{equation*}
...
\end{enumerate}
Given these definitions, they use localisation to prove the following formula \cite[Theorem 5.4.1]{CF-K-wallcrossing}:
\[
...
\]
see \cite[Corollary 5.5.1]{CF-K-wallcrossing}. Notice that the restriction ${S^\epsilon(z)(\mathbbm 1_Y)}|_{\mathbf{t}=0}$ that appears on the right-hand side of this formula coincides with what we have called $\tilde{S}^Y_0(z,q)$ above.
...
Recall that in our context $Y \hookrightarrow X$ is a very ample hypersurface and $X$ is a toric Fano variety; furthermore $\epsilon=0^+$. We have the following diagram:
...
\begin{equation*} \iota_*\virt{\QG{0}{0}{Y}{\beta}}=e(\pi_* E^Y_{0,0,\beta})\cap \virt{\QG{0}{0}{X}{\beta}} \end{equation*} 
as $\Gm$-equivariant classes, where $\pi$ is the universal curve on $\QG{0}{0}{X}{\beta}$ and $E^Y_{0,0,\beta}$ is the equivariant line bundle associated to $\mathcal O_X(Y)$\footnote{This is analogous to the bundle $L_Y$ constructed in the definition of relative quasimaps; see \S \ref{Subsection relative stable quasimaps}.}.

Since there are no markings, stability implies that the source curve of any parametrised quasimap is irreducible. Thus the fibres of $\pi$ are irreducible and so we have the following splitting:
\begin{equation*}
  e(\pi_* E^Y_{0,0,\beta})=\prod_{j=0}^{Y\cdot\beta} c_1(\sigma_0^* E^Y_{0,0,\beta}\otimes \omega_{\pi}^{\otimes j})
\end{equation*}
...
The small $J^{0^+}$-function for toric varieties has been evaluated by Givental \cite{Givental-equivariantGW}\cite[Definition 7.2.8]{CF-K}:
 \[
...
Since $X$ is Fano, $J^X_{|{\bf t}=0}=S^X_{|{\bf t}=0}(\mathbbm 1_X)$.

The coefficient $\langle[pt_X]\psi_1^{Y\cdot\beta-1},\mathbbm 1_X\rangle_{\Q{0}{2}{X}{\beta}}$ that appears in our $P$-series (multiplied by $(Y\cdot\beta)!$), can be deduced from the expansion of $S^X_{|{\bf t}=0}(\mathbbm 1_X)$ given above, and it turns out to be
\[
