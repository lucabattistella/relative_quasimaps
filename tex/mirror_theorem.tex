...
\]
see \cite[Corollary 5.5.1]{CF-K-wallcrossing}. We claim that this is equivalent to Theorem \ref{Theorem Quantum Lefschetz}. More precisely:
\begin{itemize}
\item $S^{\epsilon}(\mathbf{t},z)(\mathbbm{1}_Y)|_{\mathbf{t}=0} = \tilde{S}^Y_0(z,q)$
\item $J^{\epsilon}|_{\mathbf{t}=0} = \sum_{\beta \geq 0} \prod_{j=0}^{Y \cdot \beta} (Y + jz) S_0^X(z,\beta)$
\item $\langle [\pt_Y], P^{\epsilon}_{\mathbf{t}=0} \rangle = P_0^X(q)$
\end{itemize}
Equation (1) is clear from the definition of $\tilde{S}^Y_0(z,q)$. To show equation (2), let us look more closely at:
...
We would like to pull-back this  this Euler class. Since there are no markings, stability implies that the source curve of any parametrised quasimap is irreducible. Thus the fibres of $\pi$ are irreducible and so for any $j \geq 0$ we have an exact sequence
\begin{equation*} 0 \to \pi_* (E^Y_{0,0,\beta}(-j\sigma_{\bullet})) \to \pi_* E^Y_{0,0,\beta} \to \sigma_{\bullet}^*\mathcal{P}^{j-1}(E^Y_{0,0,\beta}) \to 0 \end{equation*}
where $\mathcal{P}^{j-1}$ denotes bundle of $(j-1)$-jets, and $\sigma_{\bullet}$ is the section given by the point $\infty \in \PP^1$ of the parametrised component. The last map is defined by evaluating a section of $E_{0,0,\beta}^Y$ at the point $\infty$ (as well as its derivatives up to order $j-1$). On the other hand we have
...
\begin{equation*}
e(\pi_* E^Y_{0,0,\beta})=\prod_{j=0}^{Y\cdot\beta} c_1(\sigma_{\bullet}^* E^Y_{0,0,\beta}\otimes \omega_{0}^{\otimes j})
\end{equation*}
Let us describe the points of $F_0^X$. The source curve is irreducible with no nodes and markings. The fact that it is $\Gm$-fixed means that the degree must be absorbed by the basepoints, i.e. that the residual is map is constant. Furthermore we can only have basepoints at $0$ and $\infty$. Then $F_0^X$ is the fixed locus at which all the degree is concentrated at $\infty$, i.e. $\infty$ is a big basepoint and $0$ is not. So we have $\ev_{\bullet} = \ev_0$.

Restricting this to $F_0^X$ and using the fact that $\omega_0$ is trivial but with weight $z$, we obtain 
