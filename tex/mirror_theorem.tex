...
where $\mathbf{t} \in \HH^*(X)$ and $z$ is the $\Gm$-equivariant parameter. Here we view $\ev_1$ as mapping to $Y \times \PP^1$, and $p_\infty\in \HH^*_{\Gm}(\PP^1)$ is the equivariant cohomology class defined by the setting $p_{\infty}|_0 =0$ and $p_{\infty}|_{\infty}=-z$ (where $0$ and $\infty$ are the $\Gm$-fixed points of $\PP^1$). Thus $\ev_1^* p_\infty$ should be thought of as a Kronecker delta function: it is zero if the first marked point lies over $0$, and $-z$ if it lies over $\infty$.
\end{enumerate}
