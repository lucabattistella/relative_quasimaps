...
Notice that we only have insertions from $i^*\HH^*(X) \subseteq \HH^*(Y)$. On the other hand
\begin{equation*} \vdim \Q{0}{(m,0)}{X|Y}{\beta}=\dim X-3 -K_X \cdot \beta +2-m \geq \dim X -1 \end{equation*}
