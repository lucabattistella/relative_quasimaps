...
Thus, we calculated $T_{0,(m)}^{X|Y}(z,\beta)$ for all $m$; substituting into equation \eqref{eqn:G} we obtain
...
\end{align*}
where the third term only appears if $K_Y \cdot \beta=0$. In particular when $Y$ is Fano only the first term occurs, and we have
\begin{equation*}
 \prod_{j=0}^{Y\cdot\beta}(Y+jz) S_0^X(z,\beta)= \sum_{i=1}^k \left\langle\dfrac{\rho_i}{z-\psi_1}, \mathbbm 1_{Y} \right\rangle_{0,2,\beta}^Y \eta^i = i_*S_0^Y(z,\beta)
\end{equation*}
in which case it is clear that Theorem \ref{Theorem Quantum Lefschetz} holds with $P_0^X(q)=1$.

More generally it is clear from the expression above that equation \eqref{eqn:mirror} in the statement of Theorem \ref{Theorem Quantum Lefschetz} holds with:
...
To prove the theorem it thus remains to show that:
