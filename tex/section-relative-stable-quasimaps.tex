...
We briefly recall the definition and basic properties of the moduli space of quasimaps; see \cite{CF-K} for more details.
...
\subsection{Review of relative stable maps} \label{Subsection relative stable maps} Given a smooth projective variety $X$ and a smooth divisor $Y$, the moduli space of relative stable maps parametrises stable maps to $X$ with specified tangency multiplicities at the marked points.
...
\end{enumerate}
\end{definition}
Note that in the case of maximal multiplicity $\Sigma_{i} \alpha_i = Y \cdot \beta$, all of the inequalities in the above definition must in fact be equalities.

This forms a proper closed substack of $\M{0}{n}{X}{\beta}$. In the case $X=\PP^N$, $Y=H$ one can show it is irreducible of the correct dimension, and hence admits a fundamental class. More generally if $Y \subseteq X$ is very ample one can use the embedding $X \hookrightarrow \PP^N$ to obtain a cartesian diagram:
\bcd
\M{0}{\alpha}{X|Y}{\beta} \ar[r] \ar[d] \ar[rd,phantom,"\square"] & \M{0}{\alpha}{\PP^N|H}{d} \ar[d] \\
\M{0}{n}{X}{\beta} \ar[r,"\varphi"] & \M{0}{n}{\PP^N}{d}
\ecd
Then the fact that $\M{0}{n}{\PP^N}{d}$ is smooth allows us to define a virtual class on $\M{0}{\alpha}{X|Y}{\beta}$ by virtual (or diagonal) pull-back:
...
where $D_{\alpha,k}(X,\beta)$ is an appropriate \ilemph{comb locus}. Repeated application of this result shows that the relative Gromov--Witten invariants and the Gromov--Witten invariants of $Y$ are completely determined by the Gromov--Witten invariants of $X$.

This relation is worked out explicitly in some cases of particular interest in \cite{Ga-MF} to obtain a new proof of the mirror theorem.
