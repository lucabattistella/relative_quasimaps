...
Consider the line bundle $\OO(Y)$ and the section $s$ cutting out $Y$. By \cite{CoxRing} we have a natural isomorphism
\begin{equation*} \HH^0(X,\OO(Y)) = \kfield \left\langle \prod_{\rho \in \Sigma_X(1)} z_\rho^{a_\rho} : \Sigma_\rho a_\rho D_\rho = Y \right\rangle \end{equation*}
where the $z_\rho$ are the generators of the Cox ring and the $a_\rho$ are arbitrary non-negative integers. We can therefore write $s$ as
\begin{equation*} s = \sum_{\underline{a}=(a_\rho)} \lambda_{\underline{a}} \prod_\rho z_\rho^{a_\rho} \end{equation*}
...
\begin{equation*} u := \sum_{\underline{a}} \lambda_{\underline{a}} \prod_\rho u_\rho^{a_\rho} \end{equation*}
...
\begin{equation*} u \in \HH^0(C,L_Y) \end{equation*}
...
to be the locus of quasimaps such that, if $Z$ is a connected component of the vanishing locus of $u$ in $C$, then
\begin{enumerate}
\item if $Z$ is a point and is equal to a marked point $x_i$, then the order of vanishing of $u$ at $x_i$ is greater than or equal to $\alpha_i$;
\item if $Z$ is one-dimensional (hence a union of irreducible components) and if we let $C^{(i)}$ for $1 \leq i \leq r$ denote the irreducible components of $C$ adjacent to $Z$, and $m^{(i)}$ the order of vanishing of $u$ at the node $Z \cap C^{(i)}$, then we must have:
\begin{equation*} \deg L_Y|_Z + \sum_{i=1}^r m^{(i)} \geq \sum_{x_i \in Z} \alpha_i \end{equation*}
\end{enumerate}
\end{definition}

In the case where $X=\PP^N$ and $H=\{ z_0 = 0 \}$ is a hyperplane, the line bundle $L_Y$ is equal to $L$ and the section $u$ is equal to $u_0$. We will examine this case in detail in the follpwing section. 
