...
\begin{equation*} i_* : \Q{0}{n}{X}{\beta} \to \Q{0}{n}{\PP^N}{d} \end{equation*}
...
\bcd
\Q{0}{\alpha}{X|Y}{\beta} \ar[r, "f", hook] \ar[d, "g", hook] \ar[dr, phantom, "\square"] & \Q{0}{n}{X}{\beta} \ar[d, "k", hook] \\
\Q{0}{\alpha}{\PP^N|H}{d} \ar[r, "j", hook] & \Q{0}{n}{\PP^N}{d}
\ecd
...
\ecd
which is equivalent to the previous one. Then we define:
\begin{equation*} \virt{\Q{0}{\alpha}{X|Y}{\beta}} := \Delta^! \left( \virt{\Q{0}{n}{X}{\beta}} \times [\Q{0}{\alpha}{\PP^N|H}{d}] \right) \end{equation*}
The idea is to prove the recursion formula for $(X,Y)$ by pulling back the formula for $(\PP^N,H)$ along $k = i_*$. In order to do this, we need to understand how various virtual classes pull back along this map. This is the technical heart of the proof.

First, we claim that for any $\alpha$ we have:
\begin{equation*} k^! [\Q{0}{\alpha}{\PP^N|H}{d}] = f_* \virt{\Q{0}{\alpha}{X|Y}{\beta}} \end{equation*}
inside $A_* \left(\Q{0}{n}{X}{\beta}\right)$. To see this, consider the following cartesian diagram:
\bcd
\Q{0}{\alpha}{X|Y}{\beta} \ar[r,"f \times g"] \ar[d,"f"] \ar[rd,phantom,"\square"] & \Q{0}{n}{X}{\beta} \times \Q{0}{\alpha}{\PP^N|H}{d} \ar[d,"\operatorname{Id} \times i"] \\
\Q{0}{n}{X}{\beta} \ar[r, "\operatorname{Id} \times k"]  \ar[d,"k"] \ar[rd,phantom,"\square"] & \Q{0}{n}{X}{\beta} \times \Q{0}{n}{\PP^N}{d} \ar[d,"k \times \operatorname{Id}"]  \\
\Q{0}{n}{\PP^N}{d} \ar[r,"\Delta"] & \Q{0}{n}{\PP^N}{d} \times \Q{0}{n}{\PP^N}{d}
\ecd
Then by compatiblity of Gysin maps with push-forwards we have:
\begin{align*} f_*\virt{\Q{0}{\alpha}{X|Y}{\beta}} & = f_* \Delta^! \left( \virt{\Q{0}{n}{X}{\beta}} \times [\Q{0}{\alpha}{\PP^N|H}{d}] \right)\\
& = (\Id \times k)^! (\Id \times i)_* \left( \virt{\Q{0}{n}{X}{\beta}} \times [\Q{0}{\alpha}{\PP^N|H}{d}] \right) \\
& = (\Id \times k)^! \left( \virt{\Q{0}{n}{X}{\beta}} \times [\Q{0}{\alpha}{\PP^N|H}{d}] \right) \\
& = k^! [\Q{0}{\alpha}{\PP^N|H}{d}] \end{align*}
Thus, the first two terms of the recursion formula pull back easily along $k$. It remains to consider the third term, namely the virtual class of the comb locus. We wish to prove that:
\begin{equation*} k^! [D^\mathcal{Q}_{\alpha,k}(\PP^r|H,d)] = \virt{D^\mathcal{Q}_{\alpha,k}(X|Y,\beta)} \end{equation*}
We will require a number of lemmas. We begin by considering a simpler moduli space than the comb locus, which we call the \ildef{centipede locus}. We fix a partition $A=(A_0,\ldots,A_r)$ of the marked points and a partition $B=(\beta_0, \ldots, \beta_r)$ of the curve class satisfying the usual stability conditions for the comb loci, and then consider the space:
...
Notice that the comb locus lives inside this space as a closed substack. We equip this space with a virtual class in the usual way by pulling back
\begin{equation*} \virt{\Q{0}{A_0 \cup \{ q_1, \ldots, q_r \}}{X}{\beta_0}} \times \prod_{i=1}^l \virt{\Q{0}{A_i\cup\{q_i\}}{X}{\beta_i}} \end{equation*}
along $\Delta : X^r \to X^r \times X^r$. That is, we set:
\begin{equation*} \mathcal{L}(X,A,B) =  \Q{0}{A_0 \cup \{ q_1, \ldots, q_r \}}{X}{\beta_0} \times \prod_{i=1}^l \Q{0}{A_i\cup\{q_i\}}{X}{\beta_i} \end{equation*}
and then make use of the cartesian diagram:
\bcd
\mathcal{K}(X,A,B) \ar[r,"g"] \ar[d,"\ev_q"] \ar[rd,phantom,"\square"] & \mathcal{L}(X,A,B) \ar[d,"\ev_q"] \\
X^r \ar[r,"\Delta_{X^r}"] & X^r \times X^r
\ecd
On the other hand, we can consider the following cartesian diagram
\bcd
...
[FIGURE]
\end{remark}
We claim that we can obtain the virtual class of $\mathcal{K}(X,A,B)$ defined previously from a perfect obstruction theory relative to the morphism $\psi \circ \rho$ in the diagram above. We will then construct a compatible triple $(E_{\psi \circ \rho}, E_{\pi}, E_{\varphi})$ so that there exists a virtual pull-back morphism $\varphi^!$ such that:
\begin{equation*} \virt{\mathcal{K}(X,A,B)} = \varphi^! \virt{\Q{0}{n}{X}{\beta}} \end{equation*}
Let us begin. First we consider the diagram [REFERENCE: diagram involving L]. Consider the following commuting diagram:
\bcd
\mathcal{L}(X,A,B) \ar[rr,"\ev \times \pi"] \ar[dr,"\pi"] & & X^r \times X^r \times \MM_{0,A,B}^{\operatorname{wt}} \ar[dl,left,"\pi_2"] \\
& \MM_{0,A,B}^{\operatorname{wt}} &
\ecd
This induces an exact triangle of contangent complexes:
\begin{equation*} (\ev \times \pi)^* L_{\pi_2} = \ev^* L_{X^r \times X^r} \to L_\pi \to L_{\ev \times \pi} \end{equation*}
The product virtual fundamental class on $\mathcal{L}(X,A,B)$ is induced by a perfect obstruction theory:
\begin{equation*} E_\pi \to L_\pi \end{equation*}
On the other hand, there exists a map
\begin{equation*} \ev^* L_{X^r \times X^r} \to E_\pi \end{equation*}
whose dual is given fibrewise by the natural morphism:
\begin{equation*} H^0(C,\mathcal{F}) \to \bigoplus_q T_q X \end{equation*} \footnote{Flesh this out!}
Here we have used the Euler sequence on $X$ and the fact that none of the $q$ are basepoints. We can complete this morphism to an exact triangle and thus obtain a diagram:
\bcd
\ev^* L_{X^r \times X^r} \ar[r] \ar[d,"\Id"] & E_\pi \ar[r] \ar[d] & E_{\ev \times \pi} \ar[r] \ar[d] & \, \\
\ev^* L_{X^r \times X^r} \ar[r] & L_\pi \ar[r] & L_{\ev \times \pi} \ar[r] & \,
\ecd
\newpage
Thus obtaining a perfect obstruction theory for the morphism $\ev \times \pi$. Now let us consider the other commuting diagram
\bcd
\mathcal{L}(X,A,B) \ar[rr,"\ev \times \pi"] \ar[dr,"\ev"] & & X^r \times X^r \times \MM_{0,A,B}^{\operatorname{wt}} \ar[dl,left,"\pi_1"] \\
& X^r \times X^r &
\ecd
...
By composing the connecting homomorphism with $E_{\ev \times \pi} \to L_{\ev \times \pi}$ we obtain a morphism of exact triangles:
\bcd
\pi^* L_{\MM_{0,A,B}^{\operatorname{wt}}} \ar[r] \ar[d,"\Id"] & E_{\ev} \ar[r] \ar[d] & E_{\ev \times \pi} \ar[r] \ar[d] & \, \\
\pi^* L_{\MM_{0,A,B}^{\operatorname{wt}}} \ar[r] & L_{\ev} \ar[r] & L_{\ev \times \pi} \ar[r] & \,
\ecd
Thus we have obtained a perfect obstruction theory $E_{\ev} \to L_{\ev}$ for $\mathcal{L}(X,A,B)$ over $X^r \times X^r$ which, by the nature of its construction, induces the standard virtual class. 

Then by functoriality of the virtual class (see REFERENCE Behrend Fantechi Proposition 7.2) we have that
\begin{equation*} g^* E_{\ev} \to g^* L_{\ev} = L_{\ev} \end{equation*}
is a perfect obstruction theory on $\mathcal{K}(X,A,B)$ over $X^r$ which induces the product class:
\begin{equation*} \Delta_{X^r}^! \virt{\mathcal{L}(X,A,B)} \end{equation*}
We now aim to use this to express the same virtual class as being induced from a perfect obstruction theory over $\MM_{0,A,B}$. Consider the diagram (REFERENCE DIAGRAM INVOLVING POINTED SPACES].
...
Earlier we obtained a relative obstruction theory $E_{\ev \times \rho} \to L_{\ev \times \rho}$ on $\mathcal{L}(X,A,B)$. We can use the cartesian diagram
\bcd
\mathcal{K}(X,A,B) \ar[r,"g"] \ar[d,"\ev \times \rho"] \ar[rd,phantom,"\square"] & \mathcal{L}(X,A,B) \ar[d,"\ev \times \rho"] \\
...
\bcd
\ev^* L_{X^r} \ar[r] \ar[d,"\Id"] & E_\rho \ar[r] \ar[d] & E_{\ev \times \rho} \ar[r] \ar[d] & \, \\
\ev^* L_{X^r} \ar[r] & L_\rho \ar[r] & L_{\ev \times \rho} \ar[r] & \,
\ecd
...
On the other hand, we can construct a map:
\begin{equation*} \varphi^* E_\pi \to E_\rho \end{equation*}
[EXPLAIN HOW THIS IS DONE] Therefore we can construct an obstruction theory for the morphism $\varphi$, and we have a virtual pull-back morphism $\varphi^!_{E_\varphi}$ such that:
\begin{equation*} \varphi^!_{E_\varphi} \virt{\Q{0}{n}{X}{\beta}} = \Delta_{X^r}^! \virt{\mathcal{L}(X,A,B)} \end{equation*}

With this lemma proven, consider the following cartesian diagram:
\bcd
\mathcal{K}(X,A,B) \ar[r] \ar[d,"q"] \ar[rd,phantom,"\square"] & \mathcal{K}(\PP^N, A, i_* B) \times \Q{0}{n}{X}{\beta} \ar[d,"p \times \Id"] \\
\Q{0}{n}{X}{\beta} \ar[d,"k"] \ar[r,"k \times \Id"] \ar[rd,phantom,"\square"] & \Q{0}{n}{\PP^N}{d} \times \Q{0}{n}{X}{\beta} \ar[d,"\Id \times k"] \\
\Q{0}{n}{\PP^N}{d} \ar[r,"\Delta"] & \Q{0}{n}{\PP^N}{d} \times \Q{0}{n}{\PP^N}{d} 
\ecd
Note that $\Delta$ is a regular embedding because $\Q{0}{n}{\PP^N}{d}$ is smooth. All the other morphisms in the diagram admit relative obstruction theories as discussed above, and hence (see REFERENCE) admit virtual pullback morphisms. The following result is the analogue of Lemma REFERENCE in Gathmann REFERENCE.
\begin{lem} $\Delta^! \left( [ \mathcal{K}(\PP^N, A, i_* B)] \times \virt{\Q{0}{n}{X}{\beta}} \right) = \Delta_{X^r}^! \virt{\mathcal{L}(X,A,B)}$ \end{lem}
\begin{proof}
...
& = q_{\text{v}}^! \Delta^! \left( [\Q{0}{n}{\PP^N}{d}] \times \virt{\Q{0}{n}{X}{\beta}} \right) \\
& = q_{\text{v}}^! \Delta^! \left( [\Q{0}{n}{\PP^N}{d}] \times k_{E_k}^! [ \Q{0}{n}{\PP^N}{d} ] \right) \\
& = q_{\text{v}}^! \Delta^! (\Id \times k)_{\text{v}}^! \left( [\Q{0}{n}{\PP^N}{d}] \times [ \Q{0}{n}{\PP^N}{d}]  \right) \\
& = q_{\text{v}}^! k_{\text{v}}^! \Delta^!\left( [\Q{0}{n}{\PP^N}{d}] \times [ \Q{0}{n}{\PP^N}{d}]  \right)  \\
& = q_{\text{v}}^! k_{\text{v}}^! [ \Q{0}{n}{\PP^N}{d} ] \\
& = q_{\text{v}}^! \virt{\Q{0}{n}{X}{\beta}} \\
& = \Delta_{X^r}^! \virt{\mathcal{L}(X,A,B)}
\end{align*}
where the final equality follows by Lemma REFERENCE above.
\end{proof}
