...
\subsubsection{Application: wall-crossing} We then apply the recursion formula to relate our invariants to the relative $I$-function considered in \cite{FanTsengYou}.
...
\end{customthm}
\noindent (See \S \ref{section wallcrossing} for a precise statement.) This demonstrates that the theory of relative quasimaps provides a natural framework for studying relative mirror symmetry.

\subsubsection{Application: quasimap quantum Lefschetz} Besides the connections to relative Gromov--Witten theory, we may also apply the recursion formula to study absolute quasimap theory. By repeatedly decreasing the tangency orders, we obtain a quantum Lefschetz theorem for quasimap invariants, expressing the invariants of a hyperplane section $Y$ in terms of those of $X$. This takes two forms; first, we have a general result which holds without any special restrictions on the target geometry:
...
\begin{lemma} \label{lemma pushforward} $i_* S^Y_0(z,\beta) = \tilde{S}^Y_0(z,\beta)$. \end{lemma}
...
where $\ev_1$ is viewed as a map to $Y$ (for simplicity we leave undefined the terms for which $d=0$). Just as in Lemma \ref{lemma pushforward} we have:
...
\end{thm}
\begin{remark} The effect of applying $i^* i_*$ is to remove the terms corresponding to the non-restricted quasimap invariants. This is merely an artefact of the proof, since the recursion formula established in \S \ref{section recursion formula} only deals with restricted insertions; the statement should hold without this caveat.\end{remark}

\begin{proof}
Applying \eqref{Gathmann recursion via generating functions} repeatedly, we obtain the following formula in $\HH^*(X)$:
\begin{equation*} \left( \prod_{m=0}^{d-1} (Y+mz) \right) S^{X}_{0}(z,\beta) = \widetilde{S}_{0,d}^{X|Y}(z,\beta) + \sum_{k=0}^{d-1} \left(\prod_{m=k+1}^{d-1}(Y+mz) \right) \widetilde{T}^{X|Y}_{0,(k)}(z,\beta).\end{equation*}
...
\begin{equation}\label{halfway to comparison of I} \widetilde{S}^{X|Y}_{0,(d)}(z,\beta) = \left( \prod_{m=0}^{d-1}(Y+mz) \right) S_{0}^X(z,\beta).\end{equation}
...
\begin{align*} i^* \widetilde{S}^{X|Y}_{0,(d)}(z,\beta) & = i^* \left(J^X_\beta(z,0) \cdot \prod_{m=0}^{d-1}(Y+mz) \right) \\
& = z^{-1} \cdot i^*Y \cdot I_{\beta,\mathrm{FTY}}^{X|Y}(z,0)\\
& = z^{-1} \cdot i^* i_* I_{\beta,\mathrm{FTY}}^{X|Y}(z,0).\end{align*}
But we also have
\begin{equation*} i^* \widetilde{S}_{0,(d)}^{X|Y}(z,\beta) = i^* i_* I_\beta^{X|Y}(z,0)\end{equation*}
