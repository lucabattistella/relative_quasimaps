...
The idea is to prove the recursion formula in $\Q{0}{n}{X}{\beta}$ by pulling back the formula for $\PP^N$ relative $H$ along $k = i^!$. In order to do this, we need to understand how various virtual classes pull back along this map. This is the technical heart of the proof.

First, we claim that for any $\alpha$ we have:
\begin{equation*} k^! [\Q{0}{\alpha}{\PP^N|H}{d}] = f_* \virt{\Q{0}{\alpha}{X|Y}{\beta}} \end{equation*}
inside $A_* \left(\Q{0}{\alpha}{X|Y}{\beta}\right)$. To see this, consider the following cartesian diagram:
\bcd
...
To do so will require a number of lemmas. We begin by considering a simpler moduli space than the comb locus, which we call the \ildef{centipede locus}. We fix a partition $A=(A_0,\ldots,A_r)$ of the marked points and a partition $B=(\beta_0, \ldots, \beta_r)$ of the curve class satisfying the usual stability conditions for the comb loci, and then consider the space:
...
\bcd
\Q{0}{A_0 \cup \{ q_1, \ldots, q_r \}}{X}{\beta_0} \times_{X^r} \prod_{i=1}^r \Q{0}{A_i\cup\{q_i\}}{X}{\beta_i} \ar[r] \ar[d] \ar[rd,phantom,"\square"] & \Q{0}{A}{X}{\beta} \ar[d] \\
\MM_{0,A_0\cup\{q_1,\ldots,q_r\},\beta_0}^{\operatorname{wt}} \times \prod_{i=1}^r \MM_{0,A_i\cup\{q_i\},\beta_i}^{\operatorname{wt}} \ar[r,"f"] & \MM_{0,A,\beta}^{\operatorname{wt}}
\ecd
