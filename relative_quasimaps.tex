...
and then apply $\R \pi_*$. This is analogous to the morphism $f^* T_X \to f^* T_{\PP^N}|_X$ which is used in the stable maps setting. However the construction for quasimaps requires a little more ingenuity, because we do not have acess to a universal map $f$.
...
where $r = \operatorname{rk} \Pic X$ (implicitly we have chosen a basis for this $\Z$-module). Similarly $\mathcal{F}_{\PP^N}$ is defined on $\mathcal{C}_{\PP^N}$ by:
...
\end{tikzcd}
\end{equation}
It is simple to check (using the fact that $\ev^* L_{X^r \times X^r}$ is concentrated in degree $0$) that $E_{\ev \times \pi} \to L_{\ev \times \pi}$ is a perfect obstruction theory for the morphism $\ev \times \pi$.
...
Let us move over to $\mathcal{K}(X,A,B)$. We can use the cartesian diagram
\bcd
...
\ecd
to pull back the obstruction theory $E_{\ev \times \pi} \to L_{\ev \times \pi}$ to an obstruction theory $E_{\ev \times \rho} \to L_{\ev \times \rho}$, which induces $\virt{\mathcal{K}(X,A,B)}$ as defined earlier (REFERENCE BEHREND FANTECHI).

We want to use this to obtain an obstruction theory for $\mathcal{K}(X,A,B)$ over $\MM_{0,A,B}^{\operatorname{wt}}$. As in the previous argument we can consider the composition
...
\end{tikzcd}
\end{equation}
and it is simple to check that $E_\rho \to L_\rho$ is a perfect obstruction theory for $\mathcal{K}(X,A,B)$ over $\MM_{0,A,B}^{\operatorname{wt}}$ which by construction induces the class $\virt{\mathcal{K}(X,A,B)}$.

Finally we wish to obtain a perfect obstruction theory for $\mathcal{K}(X,A,B)$ over $\MM_{0,n,\beta}^{\operatorname{wt}}$. We can compose this map with the connecting homomorphism $L_\rho \to \rho^* L_\psi[1]$ to obtain a morphism of exact triangles:
\bcd
\rho^* L_\psi \ar[d,"\Id"] \ar[r] & E_{\rho \circ \psi} \ar[r] \ar[d] & E_\rho \ar[r,"{[1]}"] \ar[d] & \, \\
\rho^* L_\psi \ar[r] & L_{\rho \circ \psi} \ar[r] & L_\rho \ar[r,"{[1]}"] & \,
\ecd
We wish to prove that $E_{\rho \circ \psi} \to L_{\rho \circ \psi}$ is a perfect obstruction theory. We claim that $\rho^* L_{\psi}$ is concentrated in degrees $[-1,0]$. To see this, consider the exact triangle of cotangent complexes associated to the morphism $\psi$:
...
We must show that the first map is surjective. But this is dual to the map which takes infinitesimal automorphisms of the disconnected curve to infinitesimal automorphisms of the corresponding connected curve (obtained by glueing the ``nodal'' marked points together). Since these two curves have the same set of components, this map is an isomorphism. Hence $\h^1(L_\psi) = 0$ as claimed.
...
It then follows formally that $E_{\psi \circ \rho} \to L_{\psi \circ \rho}$ is a perfect obstruction theory for $\mathcal{K}(X,A,B)$ over $\MM_{0,n,\beta}^{\operatorname{wt}}$. This completes the proof of the first part of the lemma.

To prove the second part, we must construct a map
\begin{equation*} \varphi^* E_\pi \to E_{\psi \circ \rho} \end{equation*}
By the diagram above, to construct such a morphism is the same thing as to construct a morphism $\varphi^* E_{\pi} \to E_\rho$ such that the composition
