...
The study of projective space relative a hyperplane plays a fundamental role: in this case, the relative space is irreducible of the expected codimension $\sum\alpha$ (in fact, it is the closure of the so-called ``nice locus'' consisting of maps from a $\PP^1$ whose image is not contained in $H$ and satisfying the tangency requirement). Thus it has an actual fundamental class, which we can use to define relative quasimap invariants, and to endow $\Q{0}{\alpha}{X|Y}{\beta}$ with a virtual class by pullback.
...
\begin{definition} Let $n\geq 2$ be a number of marked points, $d\in\mathbb N$ a degree, and $\alpha=(\alpha_1, \ldots, \alpha_n)\in\mathbb N^n$ a tangency requirement such that $\Sigma_i \alpha_i \leq d$.
...
\begin{definition} With notation as above, $\beta \in \HH_2^+(X)$ an effective curve class and $\alpha\in\mathbb N^n$ such that $\sum_i\alpha_i\leq\OO_X(Y)\cdot\beta$. The \ilemph{space of relative stable quasimaps}
...
\begin{remark}
The relative quasimap invariants of $(\PP^N,H)$ agree with the relative Gromov--Witten invariants of $(\PP^N,H)$, since the birational map $\chi_\alpha$ preserves the fundamental classes (also, the psi classes pull back along $\chi_\alpha$, see Lemma \ref{lem:compare_psi}). Note that if we set $\alpha=(0,\ldots,0)$ we recover the classical comparison theorem for the absolute Gromov--Witten and quasimap invariants of projective space.
\end{remark}
...
\end{lem}
\begin{proof}
The contraction of rational tails happens away from any marking.
\end{proof}
