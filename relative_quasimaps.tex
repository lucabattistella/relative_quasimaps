...
\begin{remark}
The relative quasimap invariants of $(\PP^N,H)$ agree with the relative Gromov--Witten invariants of $(\PP^N,H)$, since the map $\chi_\alpha$ is birational and hence preserves the fundamental classes (the psi classes pull back along $\chi_\alpha$ because the stabilisation of the source curve occurs away from the markings). Note that if we set $\alpha=(0,\ldots,0)$ we recover the classical comparison theorem for the absolute Gromov--Witten and quasimap invariants of projective space.
\end{remark}
...
\item if $Z$ is a subcurve of $C$ (in which case it is a union of irreducible components) and if we let $C^{(i)}$ for $1 \leq i \leq r$ denote the irreducible components of $C$ adjacent to $Z$, and $m^{(i)}$ the multiplicity of $u_Y|_{C^{(i)}}^*(0)$ at the unique node $Z \cap C^{(i)}$, then:
