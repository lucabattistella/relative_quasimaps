...
The theory of relative stable maps occupies a central place in modern enumerative geometry, due both to its intrinsic interest and its role in the degeneration formula. In this paper we take the first steps towards an extension of this theory to the setting of quasimaps. The two main applications we have in mind are:
\begin{enumerate}
\item a dramatically simplified degeneration formula, which could be used to derive closed formulae for generating functions;
\item an extension of mirror symmetry to the relative setting.
\end{enumerate}
In this paper, we carry out this program in the setting of genus-zero quasimaps relative a hyperplane section. We construct moduli spaces of relative quasimaps in the spirit of A. Gathmann, that is, as substacks of moduli spaces of (absolute) quasimaps. We prove a recursion formula for the resulting invariants which plays the role of the degeneration formula. , and use this to derive the following quantum Lefschetz result:
\begin{customthm}{A} Let $X$ be a smooth projective toric variety and $Y \subseteq X$ a smooth very ample hypersurface. Then there is an explicit algorithm to recover the (restricted) quasimap invariants of $Y$ from the quasimap invariants of $X$.\end{customthm}
\noindent Under some semipositivity assumptions on $X$ and $Y$, we are able to apply this algorithm to obtain an explicit relation between generating functions:
...
\end{customthm}
\noindent Finally, we apply our recursion to show that our relative quasimap invariants provide the correct general context for studying relative mirror symmetry:
\begin{customthm}{C} The relative $I$-function, as constructed in \cite{FanTsengYou}, coincides with a certain generating function for relative quasimap invariants.
\end{customthm}


In this introductory section we briefly recall the theories of stable quasimaps and relative stable maps, thus putting our work in its proper context.
