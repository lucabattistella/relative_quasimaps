...
We are going to be interested in the following \textbf{setup}: $X$ is a smooth projective toric variety and $Y$ is a smooth \emph{very ample} hypersurface in it, satisfying the following \emph{semi-positivity assumption}, that $-K_Y$ is nef. Notice that, by adjunction, it follows from our hypotheses that $-K_X$ is positive (at least) on every effective curve class \emph{coming from $Y$}; in fact we are going to assume (in addition) that \emph{$X$ is Fano}. Let us denote by $r$ the dimension of $X$ and assume it is \emph{at least 3}.
...
Let $X$ be a toric Fano variety of dimension at least 3, and $i\colon Y\subseteq X$ a very ample hypersurface such that $-K_Y$ is nef. Then
 \[
  \frac{\sum_{\beta\geq 0} q^\beta\prod_{i=0}^{Y.\beta}(Y+iz)S_0^X(z,\beta)}{P_0(q)}= i_*S_0^Y(z,q)
 \]
where
\begin{align*}
...
\[
 \prod_{i=0}^{Y.\beta}(Y+iz) S_0^X(z,\beta) = \sum_{m=0}^{Y.\beta}\prod_{i=m+1}^{Y.\beta}(Y+iz)T_{(m)}^{X|Y}(z,\beta).
\]
...
\[
 \prod_{i=0}^{Y.\beta}(Y+iz) S_0^X(z,\beta)= \sum_{i=1,\ldots,k;j\geq 0}z^{j+1}\eta^i\langle \rho_i\psi_1^j,\mathbbm 1_{Y}\rangle_{\Q{0}{2}{Y}{\beta}} = i_*S_0^Y(z,\beta).
\]
...
\begin{cor}
 ...
 \[
  \sum_{\beta\geq 0} q^\beta\prod_{i=0}^{Y.\beta}(Y+iz)S_0^X(z,\beta) = i_*S_0^Y(z,q)
 \]
\end{cor}
...
where
\[
 I_{\text{small}}^{Y_5}(z,q)=5H+\sum_{d>0}\frac{\prod_{i=0}^{5d}(H+iz)}{\prod_{i=0}^{d}(H+iz)^5}q^d
\]
and
\[
