...
\begin{remark} Note that $k$ is a regular embedding, since its image can be described as the zero locus of a section of the bundle
\begin{equation*} \R \pi_* \left( \bigoplus_i \mathcal{L}^{\otimes a_i} \right) \end{equation*}
on $\Q{0}{n}{\PP^N}{d}$, where $(a_i)$ is the multidegree of $X$ in $\PP^N$ (the embedding $X \hookrightarrow \PP^N$ being regular because both source and target are smooth). So the Gysin map $k^!$ is well-defined. \end{remark}


\begin{lem} \label{Relative spaces pull back} For any $\alpha$ we have:
\begin{equation*} k^! [\Q{0}{\alpha}{\PP^N|H}{d}] = \virt{\Q{0}{\alpha}{X|Y}{\beta}} \end{equation*}
inside $A_* (\Q{0}{n}{X}{\beta})$. \end{lem}

\begin{proof} Here of course by $\virt{\Q{0}{\alpha}{X|Y}{\beta}}$ inside $A_* (\Q{0}{n}{X}{\beta})$ we really mean $f_* \virt{\Q{0}{\alpha}{X|Y}{\beta}}$. Consider the following cartesian diagram:
\bcd
\Q{0}{\alpha}{X|Y}{\beta} \ar[r,"f \times g"] \ar[d,"f"] \ar[rd,phantom,"\square"] & \Q{0}{n}{X}{\beta} \times \Q{0}{\alpha}{\PP^N|H}{d} \ar[d,"\operatorname{Id} \times j"] \\
...
& = (\Id \times k)^! (\Id \times j)_* \left( \virt{\Q{0}{n}{X}{\beta}} \times [\Q{0}{\alpha}{\PP^N|H}{d}] \right) \\
& = (\Id \times k)^! \left( \virt{\Q{0}{n}{X}{\beta}} \times [\Q{0}{\alpha}{\PP^N|H}{d}] \right) \\
& = k^! [\Q{0}{\alpha}{\PP^N|H}{d}]
\end{align*}
\end{proof}
...
\begin{equation*} k^! [D^\mathcal{Q}_{\alpha,k}(\PP^N|H,d)] = \virt{D^\mathcal{Q}_{\alpha,k}(X|Y,\beta)} \end{equation*} \end{lem}
...
\begin{equation*} \virt{\mathcal{K}(X,A,B)} = k^!_{\text{v}} [ \mathcal{K}(\PP^N,A,B)] \end{equation*}
from which it follows that:
\begin{align*} \virt{D(X|Y)} & = f^! k^!_{\text{v}} [ \mathcal{K}(\PP^N,A,B) ] \\
& = k^!_{\text{v}} i^! [ \mathcal{K}(\PP^N,A,B) ] \\
& = k^!_{\text{v}} [ D(\PP^N|H) ] \end{align*}
...
\begin{proof} Pull-back the equation for $\PP^N$ relative $H$ using the map $k^!_\text{v}$, applying Lemmas \ref{Relative spaces pull back} and \ref{Comb loci pull back}. \end{proof}
