...
\end{tikzcd}
\end{equation}
It is simple to check (using the fact that $\ev^* L_{X^r \times X^r}$ is concentrated in degree $0$) that $E_{\ev \times \pi} \to L_{\ev \times \pi}$ is a perfect obstruction theory for the morphism $\ev \times \pi$. Now let us consider the other commuting diagram
\bcd
...
as follows. By the diagram above, to construct a morphism $\varphi^* E_\pi \to E_\rho$ is the same thing as to construct a morphism $\varphi^* E_\pi \to E_{\ev \times \rho}$ such that the composition
...
along $\ev^*$. Commutativity and the fact that the top row is an exact triangle (hence composes to zero) implies that the bottom row composes to zero also.

Thus we finally obtain a map:
\begin{equation*} \varphi^* E_\pi \to E_\rho \end{equation*}
A simple but fairly lengthy diagram chase shows that the obstruction theory $E_\varphi$ induced by this map is perfect. Hence we have a virtual pull-back morphism $\varphi^!_{\text{v}}$ such that
