...
So let $X$ and $Y$ be smooth and proper toric varieties with fans $\Sigma_X \subseteq N_X$ and $\Sigma_Y \subseteq N_Y$. Suppose we are given $\varphi : Y \to X$ (which we do not assume to be a toric morphism). By \cite[Theorem 1.1]{CoxFunctor} the data of such a map is equivalent to a $\Sigma_X$--collection on $Y$:
...
\begin{thm} \cite[Theorem 2.2]{CoxFunctor} The data of a morphism $Y \to X$ is the same as the data of homogeneous polynomials
...
for all $\rho \in \Sigma_X(1)$. Finally, if we define $\tilde{\varphi}(z_\tau) = (P_\rho(z_\tau))$ then this defines a lift of $\varphi$ to the prequotients:
\bcd
\Aaff_k^{\Sigma_Y(1)} - Z(\Sigma_Y) \ar[r, "\tilde{\varphi}"] \ar[d, "\pi"] & \Aaff_k^{\Sigma_X(1)} - Z(\Sigma_X) \ar[d,"\pi"] \\
Y \ar[r, "\varphi"] & X
\ecd
\end{thm}
...
Recall our goal: given a map $\varphi : Y \to X$ we wish to define a ``push-forward'' map:
\begin{equation*} \varphi_* : \Q{g}{n}{Y}{\beta} \to \Q{g}{n}{X}{\varphi_*\beta} \end{equation*}
Consider then a quasimap $(C, (L_\tau, u_\tau)_{\tau \in \Sigma_Y(1)}, (\varphi_{m_Y})_{m_Y \in M_Y})$ with target $Y$.
