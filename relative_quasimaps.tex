...
\author{Luca Battistella\footnote{Max-Planck-Institut f\"ur Mathematik, Bonn}\, and Navid Nabijou\footnote{School of Mathematics and Statistics, University of Glasgow}}
...
The plan of the paper is as follows. In \S\S \ref{Subsection stable quasimaps} and \ref{Subsection relative stable maps} we provide a brief review of the theories of stable quasimaps and relative stable maps. Then in \S \ref{Subsection relative stable quasimaps} we define the space of relative quasimaps as a substack of the moduli space of (absolute) quasimaps:
...
In \S \ref{Section recursion for PN} we examine the special case of a hyperplane $H \subseteq \PP^N$. We find that the relative space is irreducible of the expected dimension (in fact, more than this: it is the closure of the so-called ``nice locus'' consisting of maps from a $\PP^1$ whose image is not contained in $H$). Thus it has an actual fundamental class, which we can use to define relative quasimap invariants.
...
\begin{remark}\label{rmk:epsilonstable}
...
\subsection{Review of relative stable maps} \label{Subsection relative stable maps} Given a smooth projective variety $X$ and a smooth very ample divisor $Y$, Gathmann's space of relative stable maps parametrises stable maps to $X$ with specified tangencies to $Y$ at the marked points.

\begin{definition}\label{def:Garelmap} {\cite[Definition 1.1]{Ga}}  Let $X$ be a smooth projective variety and $Y \subseteq X$ a smooth very ample divisor. Fix a number $n \geq 0$ of marked points, an effective curve class $\beta \in \HH_2^+(X)$ and an $n$-tuple $\alpha = (\alpha_1, \ldots, \alpha_n)$ of non-negative integers such that $\Sigma_i \alpha_i \leq Y \cdot \beta$. The space
\begin{equation*} \M{0}{\alpha}{X|Y}{\beta} \end{equation*}
of relative stable maps to $(X,Y)$ consists of those points $(C, (x_i \in C)_{i=1}^n , f : C \to X)$ in $\M{0}{n}{X}{\beta}$ satisfying the following two conditions:
\begin{enumerate}
\item if $x_i$ is a marked point such that $\alpha_i > 0$ then $f(x_i) \in Y$;
\item if we consider the class $f^* [Y] \in \Achow_0(f^{-1}Y)$ then the difference $f^* [Y] - \sum_i \alpha_i [x_i]$ is an effective class.
\end{enumerate}
...
There is an equivalent, more combinatorial definition which is more useful in practice (see \cite[Remark 1.4]{Ga}): a stable map $(C,x_1, \ldots, x_n,f)$ is a relative stable map if and only if, for each connected component $Z$ of $f^{-1}(Y) \subseteq C$:
\begin{enumerate}
...
From here on $X$ will denote a smooth projective toric variety and $Y \subseteq X$ a very ample hypersurface. We \emph{do not} require that $Y$ is toric.
...
We now give the main definition of the paper. We start from the unbstructed case $(\PP^N|H=\{z_0=0\})$. The above discussion simplifies.
\begin{definition}
 The space of relative quasimaps $\Q{0}{\alpha}{\PP^N|H}{d}$ is the closure inside $\Q{0}{n}{\PP^N}{d}$ of the nice locus of quasimaps \[\left((\PP^1,x_1,\ldots,x_n),u_0,\ldots,u_N\in H^0(\PP^1,\OO_{\PP^1}(d))\right)\] such that $u_0\not\equiv 0$, $u_0^*(0)\geq\sum\alpha_ix_i$ and $(u_0,\ldots,u_N)$ do not vanish simultaneously on $\PP^1$.
 \end{definition}
\noindent We use Fulton's refined Gysin map \cite[\S 2.6]{FUL} to define $u_0^*(0) := 0^!([C])\in\Achow_*(u_0^{-1}(0))$ from the diagram:
\bcd
u_0^{-1}(0) \ar[r] \ar[d] \ar[rd,phantom,"\square"] & \PP^1 \ar[d,"u_0"] \\
\PP^1 \ar[r,"0"] & \operatorname{Vec}(\OO_{\PP^1}(d))
\ecd

More generally, let $X$ and $Y$ be as above. The complete linear system associated to $\OO_X(Y)$ defines an embedding $i \colon X \hookrightarrow \PP^N$ such that $i^{-1}(H) = Y$ for a certain hyperplane $H$. By the functoriality property of quasimap spaces (see \cite[\S 3.1]{CF-K-wallcrossing}) we have a map:
\begin{equation*} \om{Q}(i) \colon \Q{0}{n}{X}{\beta} \to \Q{0}{n}{\PP^N}{d} \end{equation*}
...
\begin{definition} With notation as above, let $n \geq 2$ be a number of marked points, $\beta \in \HH_2^+(X)$ be an effective curve class and $\alpha=(\alpha_1, \ldots, \alpha_n)$ be a collection of non-negative integers such that $\Sigma_i \alpha_i \leq Y \cdot \beta$. The \ilemph{space of relative stable quasimaps}
\begin{equation*} \Q{0}{\alpha}{X|Y}{\beta} \subseteq \Q{0}{n}{X}{\beta} \end{equation*}
is defined to be the following fiber product:
\bcd
\Q{0}{\alpha}{X|Y}{\beta}\ar[d]\ar[r,hook]\ar[dr,phantom,"\Box"] & \Q{0}{n}{X}{\beta}\ar[d,"\om{Q}(i)"] \\
...
\begin{lemma}
 $\Q{0}{\alpha}{\PP^N|H}{d}$ is irreducible of codimension $\sum\alpha$ in $\Q{0}{n}{\PP^N}{d}$.
\end{lemma}
\begin{proof}
 It is enough to show the statement holds for the nice locus of which we are taking the closure. Since we are requiring that there are no basepoints, such locus is also a locally closed substack of $\M{0}{n}{\PP^N}{d}$. The proof of \cite[Lemma 1.8]{Ga} applies.
\end{proof}
Indeed there exists a comparison morphism:
\begin{equation*}\chi : \M{0}{n}{\PP^N}{d} \to \Q{0}{n}{\PP^N}{d} \end{equation*}
Recall that $\chi$ has the effect of contracting every rational tail and introducing a basepoint at the corresponding node, with multiplicity equal to the degree of the rational tail. For more details, see \cite[Theorem 3]{MOP} and \cite[Section 4.3]{Manolache-Push} (earlier manifestations of these ideas can be found in \cite{Bertram} and \cite{Popa-Roth}).
...
\end{lemma}
\begin{proof}
 $\chi(\M{0}{\alpha}{\PP^N|H}{d})\subseteq \Q{0}{\alpha}{\PP^N|H}{d}$ follows by applying $\chi$ to a smoothing family of a relative map, while the reverse inclusion follows because $\chi$ is proper (hence closed). Notice that $\M{0}{\alpha}{\PP^N|H}{d}$ embeds as a closed substack of the fiber product $\chi^{-1}(\Q{0}{\alpha}{\PP^N|H}{d})$ (where the map along rational tails is sufficiently ramified at $H$), hence $\chi_\alpha$ is also proper.
\end{proof}
...
\begin{remark}
 Relative Gromov-Witten and quasimap invariants of $(\PP^N|H)$ coincide on the nose: $\chi$ being birational, it preserves the fundamental class; furthermore, any stabilisation of the source curve happens away from the markings, hence $\psi$-classes pullback under $\chi$. Setting $\alpha=0$ recovers the absolute case.
\end{remark}

Since $\Q{0}{n}{\PP^N}{d}$ is unobstructed, the morphism $\om{Q}(i)\colon\Q{0}{n}{X}{\beta}\to \Q{0}{n}{\PP^N}{d}$ admits a natural perfect obstruction theory, and so there is a virtual pull-back morphism $\om{Q}(i)^!_{\operatorname{v}}$. Alternatively, we may use the presence of a virtual class on $\Q{0}{n}{X}{\beta}$ to define a diagonal pull-back morphism $\om{Q}(i)^!_{\Delta}$. The discussion in Appendix \ref{appendix:intersection} shows that these two maps coincide, and from now on we will denote them both by $\om{Q}(i)^!$.

We now define the virtual class on $\Q{0}{\alpha}{X|Y}{\beta}$ by pull-back along $\om{Q}(i)$
\begin{equation*} \virt{\Q{0}{\alpha}{X|Y}{\beta}} := \om{Q}(i)^! [ \Q{0}{\alpha}{\PP^N|H}{d} ] \end{equation*}
and use this class to define relative quasimap invariants in general.

Recall the discussion at the beginning of Section \ref{Subsection relative stable quasimaps}. We may now use $\chi$ to identify the points of $\Q{0}{\alpha}{X|Y}{\beta}$ more explicitly.
\begin{lem}[Combinatorial description] A quasimap belongs to the relative space $\Q{0}{\alpha}{X|Y}{\beta}\subseteq \Q{0}{n}{X}{\beta}$ if and only if every connected component $Z$ of $u_Y^{-1}(0)$ satisfies the following:
\begin{enumerate}
\item if $Z$ is a point and is equal to a marking $x_i$, then $u_Y^*(0)$ has order at least $\alpha_i$ at $x_i$;
...
\end{lem}
\begin{proof}
 From the definition of $\Q{0}{\alpha}{X|Y}{\beta}$, we see that it is enough to prove the characterisation in the case $(X|Y)=(\PP^N|H)$. The statement holds on the nice locus by definition, so it holds on its closure $\Q{0}{\alpha}{\PP^N|H}{d}$ by the principle of conservation of number. On the other hand, let $\xi'$ be a quasimap satisfying the combinatorial description, with a basepoint $q_0$ of order $d_0$ on an internal component $Z$. Choose any line $\ell$ through the image of $q_0$, with $\ell$ transverse to $H$. Lift $\xi'$ to a stable map $\xi$ by adjoining a $\PP^1=:R$ to the source curve at $q_0$, and defining the map $f_{|R}$ to be a $d_0$-fold cover of $\ell$ totally ramified at $q_0$ (any basepoint which is not internal is easier to deal with). Then $\xi$ belongs to $\M{0}{\alpha}{\PP^N|H}{d}$ by the discussion following Definition \ref{def:Garelmap}, hence $\xi'=\chi(\xi)\in\Q{0}{\alpha}{\PP^N|H}{d}$.
\end{proof}
...
The formula is proven by pulling back the recursion for $(\PP^N|H)$ along $\om{Q}(i)$. Only the last term needs discussion at this point.
...
\subsection{Quasimap quantum Lefschetz formula} We now turn to our main result: a formula expressing the generating function $\tilde{S}_0^Y(z,q)$ for restricted quasimap invariants of $Y$ in terms of the quasimap invariants of $X$.
...
for $\mathbf{t} \in \HH^*(X)$. Here $\QGe{0}{m}{Y}{\beta}$ is the moduli space of $\epsilon$-stable quasimaps with a parametrised component - described briefly after Remark \ref{rmk:epsilonstable} -, $F_0$ is a certain fixed locus of the natural $\Gm$-action on this space, and $\ev_\bullet$ is the evaluation at the point $\infty \in \PP^1$ on the parametrised component. $\operatorname{Res}_{F_0}$ is the residue of the virtual class, i.e. the virtual class of the fixed locus divided by the Euler class of the virtual normal bundle (see \cite{GraberPandharipande} for details on virtual localisation). The variable $z$ is the $\Gm$-equivariant parameter.
