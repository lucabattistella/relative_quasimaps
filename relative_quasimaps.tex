...
\address{%
\affilnum{1}Max-Planck-Institut f\"ur Mathematik, Bonn\\
...
\begin{align*} \mathfrak{S}^X_\beta(z) & =(\ev_1)_*\left(\frac{1}{z-\psi_1} \virt{\Q{0}{2}{X}{\beta}}\right) 
\intertext{and}
S_0^X(z,q) &=\sum_{\beta\geq 0} q^\beta \mathfrak{S}^X_\beta(z)\end{align*}
where by convention $\mathfrak{S}_0^X(z) = \mathbbm 1_X$, and $q$ is a Novikov variable. These are generating functions for quasimap invariants of $X$ which take values in $\HH^*(X)$. The notation $S_0^X(z,q)$ is due to the fact that this is the $t=0$ restriction of the $S^{\small{0+}}$-operator; see \S \ref{Subsection CFK comparison} below.
...
\begin{lem} \label{lemma pushforward} $i_* S^Y_0(z,\beta) = \tilde{S}^Y_0(z,\beta)$. \end{lem}
