...
So let $X$ and $Y$ be smooth and proper toric varieties with fans $\Sigma_X \subseteq N_X$ and $\Sigma_Y \subseteq N_Y$. Suppose we are given $f : Y \to X$ (which we do not assume to be a toric morphism). By \cite[Theorem 1.1]{CoxFunctor} the data of such a map is equivalent to a $\Sigma_X$--collection on $Y$:
...
\begin{thm} \cite[Theorem 2.2]{CoxFunctor} The data of a morphism $f:Y \to X$ is the same as the data of homogeneous polynomials
...
\bcd
\Aaff_k^{\Sigma_Y(1)} - Z(\Sigma_Y) \ar[r, "\tilde{f}"] \ar[d, "\pi"] & \Aaff_k^{\Sigma_X(1)} - Z(\Sigma_X) \ar[d,"\pi"] \\
Y \ar[r, "f"] & X
\ecd
\end{thm}
...
For each $\rho$, $P_\rho$ is a polynomial in the $z_\tau$; we can write it as
...
where the sum is over certain multindices $\underline{a} = (a_\tau) \in \N^{\Sigma_Y(1)}$ and the $\mu_{\underline{a}}$ are nonzero scalars. The homogeneity of $P_\rho$ implies that for all $\underline{a}$ we have:
\begin{equation*} \sum_{\tau} a_\tau D_\tau = \beta_\rho \in \Pic Y \end{equation*}
For each $\underline{a}$ consider the following line bundle on $C$:
...
\begin{equation*} \mu_{\underline{a}} \prod_\tau u_\tau^{a_\tau} \end{equation*}
...
\bcd
0 \ar[r] & M_Y \ar[r, "\theta"] & \Z^{\Sigma_Y(1)} \ar[r] & \Pic Y \ar[r] & 0
\ecd
...
which is precisely the statement that in the above sequence $\underline{a}$ and $\underline{b}$ map to the same element of $\Pic Y$. Hence there exists an $m_Y \in M_Y$ such that:
...
\begin{equation*} \varphi_{m_Y} : \bigotimes_\tau L_\tau^{\otimes \langle m_Y, u_\tau \rangle} \cong \OO_C \end{equation*}
By definition, $\theta(m_Y) = (\langle m_Y,u_\tau)_{\tau \in \Sigma_Y(1)}$. But also $\theta(m_Y) = (a_\tau - b_\tau)_{\tau \in \Sigma_Y(1)}$. Hence we have:
...
for all $\underline{a}$ and $\underline{b}$. Let us choose one such $\underline{a}$ (it doesn't matter which) and define:
\begin{equation*} \tilde{L}_\rho = \tilde{L}_\rho^{\underline{a}} \end{equation*}
...
(Note that if we had made a different choice of $\underline{a}$ above the result would have been isomorphic.)

Thus, we have $(\tilde{L}_\rho, \tilde{u}_\rho)_{\rho \in \Sigma_X(1)}$ on $C$. It remains to define the isomorphisms
\begin{equation*} \tilde{\varphi}_{m_X} : \bigotimes_\rho \tilde{L}_\rho^{\otimes \langle m_X, u_\rho \rangle} \cong \OO_C \end{equation*}
for all $m_X \in M_X$. 
