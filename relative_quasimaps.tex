...
\subsection{An Intersection-Theoretic Lemma}
Shortly we will be dealing with morphisms which do not admit relative perfect obstruction theories, and as such we will require a slightly different version of the virtual pull-back morphism.
...
\ecd
Then we have:
\begin{align*} k_{\text{v}}^! [ \mathcal{K}(\PP^N|H) ] & = k_{\text{v}}^! \theta_{\text{v}}^! [ \mathcal{K}(\PP^N) ] = \theta_{\text{v}}^! k_{\text{v}}^! [ \mathcal{K}(\PP^N) ] \\
& = \theta_{\text{v}}^! \virt{\mathcal{K}(X)} = \virt{\mathcal{K}(X|Y)}
\end{align*}
