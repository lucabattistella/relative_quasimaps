...
\begin{customthm}{A} The relative $I$-function, as constructed in \cite{FanTsengYou}, coincides with a certain generating function for relative quasimap invariants.
\end{customthm}

We may also use the recursion formula to derive quantum Lefschetz theorems for absolute quasimap invariants. We have:
\begin{customthm}{B} Let $X$ be a smooth projective toric variety and $Y \subseteq X$ a smooth very ample hypersurface. Then there is an explicit algorithm to recover the (restricted) quasimap invariants of $Y$ from the quasimap invariants of $X$.\end{customthm}

\begin{remark} Even in the situation where the quasimap theories of $X$ and $Y$ coincide with the respective Gromov--Witten theories (for instance, if $X$ and $Y$ are projective complete intersections of Fano index $\geq 2$), the quasimap algorithm is combinatorially much simpler than the Gromov--Witten algorithm \cite[Corollary 5.7]{Ga}, due to the absence of rational tails. This phenomenon will generalise to the setting of the quasimap degeneration formula, once such a formula has been established.
\end{remark}

Under some semipositivity assumptions on $X$ and $Y$, we are able to apply this algorithm to obtain an explicit relation between generating functions:
\begin{customthm}{C}
...
\end{equation*}
where $S_0^X(z,\beta), \tilde{S}_0^Y(z,q)$ and $P_0^X(q)$ are appropriate generating functions for quasimap invariants.
\end{customthm}
